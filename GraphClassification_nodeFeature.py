...
from torch_geometric.nn import GraphConv, SAGEConv, GINConv, TopKPooling, GlobalAttention 
...
class single_head(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, ratio=0.7, min_score=0.7):
        ...
        self.conv = GraphConv(self.in_channels, self.out_channels, aggr='add', bias=True)
...
class Net(torch.nn.Module):
    def __init__(self, dataset_num_features, embedding_size, num_heads, num_blocks, num_classes):
        ...
        self.conv0 = GraphConv(self.dataset_num_features , self.in_channels, aggr='mean', bias=True)
...
dataset = TUDataset('/home/aashay/Desktop/pytorch_geometric/DDP/Datasets/TU/'+ dataset_name, 
                    name = dataset_name, 
                    use_node_attr = True,
